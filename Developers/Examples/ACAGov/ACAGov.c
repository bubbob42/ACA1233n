 ...
 *   clock via ACA12xx.library's speed stepping functions
 *   
 *   GetUsage() has been picked from the DOpus4/XOper sources
 *   
 *   generates proper ADOS returncodes for scripting
 *
 *   Marcus Gerards, 2015-2017
 * 
 *   Update (Jul 2016): added support for ACA1233n.library
 *          (Jan 2017): improved shell output
 *
 */
...
/* Version information */
char VersionString[] ="$VER: ACAGov 1.12 "__AMIGADATE__;
...
/* return variables/pointers for ACA12xx.library */
...
ULONG ag_UpperThreshold = 80;
ULONG ag_LowerThreshold = 50;
ULONG ag_Interval = 200; /* default interval = 200 milliseconds */
ULONG ag_MinStep =  ACA1221MINSTEP;  /* depends on card, but all ACAs have MINSTEP 0 */
ULONG ag_MaxStep;  
    ...
    char **argv; 
{
    ...
    if (DOSBase && (ACA1221Base || ACA1233nBase) && (argc != 0)) {
        ...
        if ((mport = FindPort("ACAGov"))) {
            ...
            if ((rda = ReadArgs(ARGS, (LONG *) argvalues, NULL)) != NULL) {
                ...
                ag_MaxStep = ACAMaxSpeed->ci_CPUSpeedStep;
            
                /* check if user's MaxStep value is valid for this card */
                if (argvalues[5]) {
                    if (*argvalues[5] > ACAMaxSpeed->ci_CPUSpeedStep) {
                        Printf("Your selected speed stepping value is too large. This card is licensed or equipped for speed stepping %ld (%s)\n",
                ...
                if (argvalues[6]) {
                    if ((*argvalues[6] < ACA1221MINSTEP) || (*argvalues[6] >= ACAMaxSpeed->ci_CPUSpeedStep)) {
                        Printf("MINSTEP must be at least 0 and smaller than the largest possible MAXSTEP for the governor to work properly! Setting MINSTEP to 0.\n");
                    } else {
                        ag_MinStep = *argvalues[6];
                    }
                }
...
cleanup:
    ...
    /* cleanup shell output */
    if (!(ag_NoOutput)) Printf("\r\033[2K\n");

   
    if (DosBase)        CloseLibrary((struct Library *) DosBase);  
    if (ACA1221Base)    CloseLibrary((struct Library *) ACA1221Base);
    if (ACA1233nBase)    CloseLibrary((struct Library *) ACA1233nBase);
 
    return (retval);
}
...
VOID ACAGov(VOID)
{
    ...
    if (ag_AutoClock) {
        if ((CPULoad > ag_UpperThreshold) && (ACASpeed->ci_CPUSpeedStep < ag_MaxStep)) {
            Forbid();
            if (ACA1221Base) {
                stepChange = SetSpeed(ACASpeed->ci_CPUSpeedStep + 1);
            }
            else {
                stepChange = ACA1233_SetSpeed(ACASpeed->ci_CPUSpeedStep + 1);
            }
            if (stepChange) {
                if (!(ag_NoOutput)) Printf("Increasing speed!\r\033[2K");
            } else {
                Permit();
        ...
        /* and decrease speed stepping by 1 if CPULoad < 40% */ 
            ...
            if (stepChange) {
                if (!(ag_NoOutput)) Printf("Decreasing speed!\r\033[2K");
            } else {
                Permit();
            ...
            if (!(ag_NoOutput)) Printf("No speed change.\r\033[2K");
        }
    }
    else {

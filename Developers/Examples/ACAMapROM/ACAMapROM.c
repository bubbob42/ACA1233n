...
/* Version information */
char VersionString[] ="$VER: ACAMapROM 1.1 "__AMIGADATE__;
    ...
    if ( DosBase && (ACA1221Base || ACA1233nBase) ) {
        ...
        if ( (rda = ReadArgs(ARGS, (LONG *) argvalues, NULL)) != NULL ) {
            if (argvalues[3]) {
                /* remove mapped ROM */
                if (ACA1221Base) {
                    //success = MapROM(ACAMAPREMOVE); /* TBI */
                }
                else {
